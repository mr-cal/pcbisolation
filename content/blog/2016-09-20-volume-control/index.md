...
bool subtractEnd()
{
  ...
  if(  (month == 11) |
       ...
       (month == 1)  |
       (month == 2)  |
       (month == 3 & dayOfMonth <=14 ) )
  {
    // then subtract an hour from the end of the day
    return true;
  }

  // else you are between 3/15 and 10/31, so do not subtract an hour
  return false;
...
/*
  ...
  {
   digitalWrite(STATUS_LED, HIGH);
   digitalPotWrite(HIGH_OUT);
  }
  else
  {
    digitalWrite(STATUS_LED, LOW);
    digitalPotWrite(LOW_OUT);
  }
*/

  // if time is past the start time and before the end time
  if( hour >= 10 - subtractBeg() & hour < 20 - subtractEnd() ) {
    digitalPotWrite(HIGH_OUT);  // then turn up the volume!
    digitalWrite(STATUS_LED, HIGH);
  }
  else {
